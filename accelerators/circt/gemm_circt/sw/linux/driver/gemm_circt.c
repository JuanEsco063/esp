...
#define DRV_NAME	"gemm_circt"

/* <<--regs-->> */
#define GEMM_BATCH_SIZE_REG 0x4c
#define GEMM_COLS1_REG 0x48
#define GEMM_COLS2_REG 0x44
#define GEMM_ROWS1_REG 0x40

struct gemm_circt_device {
...
static inline struct gemm_circt_device *to_gemm(struct esp_device *esp)
{
	return container_of(esp, struct gemm_circt_device, esp);
}

static void gemm_prep_xfer(struct esp_device *esp, void *arg)
{
	struct gemm_circt_access *a = arg;

	/* <<--regs-config-->> */
	iowrite32be(a->batch_size, esp->iomem + GEMM_BATCH_SIZE_REG);
	iowrite32be(a->cols1, esp->iomem + GEMM_COLS1_REG);
	iowrite32be(a->cols2, esp->iomem + GEMM_COLS2_REG);
	iowrite32be(a->rows1, esp->iomem + GEMM_ROWS1_REG);
...
{
	/* struct gemm_circt_device *gemm = to_gemm(esp); */
	/* struct gemm_circt_access *a = arg; */
...
static struct esp_driver gemm_driver = {
	...
	.ioctl_cm	= GEMM_CIRCT_IOC_ACCESS,
	.arg_size	= sizeof(struct gemm_circt_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("gemm_circt driver");
